    ...
    **kwargs,
    ...
    if (
        ...
        if shard_config.enable_sequence_parallelism:
            ...
            ) % shard_config.sequence_parallel_size == 0, f"Expected {txt.shape[1] +img.shape[1]} % {shard_config.sequence_parallel_size} == 0"
    ...
    def module_policy(self) -> Dict[Union[str, nn.Module], ModulePolicyDescription]:
        ...
        if self.shard_config.enable_sequence_parallelism:
            if not is_share_sp_tp(self.shard_config.sequence_parallelism_mode):
                policy[DoubleStreamBlock].attribute_replacement[
                    "processor"
                ] = DistributedDoubleStreamBlockProcessor(self.shard_config)
                policy[SingleStreamBlock].attribute_replacement[
                    "processor"
