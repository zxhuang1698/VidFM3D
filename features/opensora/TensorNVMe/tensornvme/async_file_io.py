...
class AsyncFileWriter:
    def __init__(
        self, path: str, n_entries: int = 16, backend=None, n_tasks: int = 0
    ) -> None:
        ...
        if backend is not None:
            self.io = AsyncFileWriterC(
                self.fd, n_entries, backend=backend, n_tasks=n_tasks
            )
        else:
    ...
    def write(self, data: bytes) -> int:
        ...
        self.io.write(
            ...
            len(data),
            self.offset,
            partial(AsyncFileWriter.gc_callback, self.buffers, len(self.buffers) - 1),
        )
    ...
    def write_tensor(self, tensor: Tensor, pinned: Optional[Tensor] = None) -> None:
        with torch.cuda.stream(self.comm_stream):
            ...
            self.io.write_tensor(
                ...
                self.offset,
                partial(
                    AsyncFileWriter.gc_callback, self.buffers, len(self.buffers) - 1
                ),
                pinned,
            )
