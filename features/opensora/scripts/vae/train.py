...
def main():
    ...
    if use_discriminator:
        discriminator = (
            build_module(cfg.discriminator, MODELS).to(device, dtype).train()
        )
    ...
    modulated_mixed_image_ratio = (
        num_ranks * mixed_image_ratio / (num_ranks - 1)
        if num_ranks > 1
        else mixed_image_ratio
    )
    if is_log_process(plugin_type, plugin_config):
        ...
        if (
            ...
            if cfg.get("load_optimizer", True):
                booster.load_optimizer(
                    disc_optimizer, os.path.join(cfg.load, "disc_optimizer")
                )
                if disc_lr_scheduler is not None:
                    booster.load_lr_scheduler(
                        disc_lr_scheduler, os.path.join(cfg.load, "disc_lr_scheduler")
                    )
                if cfg.get("disc_lr", None) is not None:
                    set_lr(disc_optimizer, disc_lr_scheduler, cfg.disc_lr)

        logger.info(
            "Loaded checkpoint %s at epoch %s step %s",
            cfg.load,
            start_epoch,
            start_step,
        )
            ...
            with profiler_ctxt:
                for _ in range(start_step, num_steps_per_epoch):
                    ...
                    if mixed_strategy == "mixed_video_image":
                        if (
                            random.random() < modulated_mixed_image_ratio
                            and dist.get_rank() != 0
                        ):
                    ...
                    with Timer("model", log=True) if cfg.get(
                        "profile", False
                    ) else nullcontext():
                        # == forward pass ==
                        ...
                        with Timer("backward", log=True) if cfg.get(
                            "profile", False
                        ) else nullcontext():
                            with ctx:
                                booster.backward(
                                    loss=vae_loss / accumulation_steps,
                                    optimizer=optimizer,
                                )

                        with Timer("optimizer", log=True) if cfg.get(
                            "profile", False
                        ) else nullcontext():
                    ...
                    if use_discriminator:
                        ...
                        with ctx:
                            booster.backward(
                                loss=disc_loss / accumulation_steps,
                                optimizer=disc_optimizer,
                            )
                        if (step + 1) % accumulation_steps == 0:
                            disc_optimizer.step()
                    ...
                    if (global_step + 1) % accumulation_steps == 0:
                        if (
                            ...
                            and actual_update_step % cfg.get("log_every", 1) == 0
                        ):
                            avg_loss = {
                                k: v / log_step for k, v in running_loss.items()
                            }
                            # progress bar
                            pbar.set_postfix(
                                {
                            ...
                            tb_writer.add_scalar(
                                "loss", vae_loss.item(), actual_update_step
                            )
                            # wandb
                            if cfg.get("wandb", False):
                                wandb.log(
                        ...
                        if ckpt_every > 0 and actual_update_step % ckpt_every == 0:
                            ...
                            if use_discriminator:
                                booster.save_model(
                                    discriminator,
                                    os.path.join(save_dir, "discriminator"),
                                    shard=True,
                                )
                                ...
                                if disc_lr_scheduler is not None:
                                    booster.save_lr_scheduler(
                                        disc_lr_scheduler,
                                        os.path.join(save_dir, "disc_lr_scheduler"),
                                    )
                            dist.barrier()
