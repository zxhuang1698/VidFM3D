...
def main():
    ...
    pin_memory_cache_pre_alloc_numels = cfg.get(
    ...
    exp_name, exp_dir = create_experiment_workspace(
        ...
        exp_name=cfg.get(
            "exp_name", None
        ),  # useful for automatic restart to specify the exp_name
    )
    ...
    # == build model model ==
    model = build_module(
        cfg.model, MODELS, device_map=device, torch_dtype=dtype
    ).train()
    ...
    if not cfg.get("cached_video", False):
        # == buildn autoencoder ==
        model_ae = (
            build_module(cfg.ae, MODELS, device_map=device, torch_dtype=dtype)
            .eval()
    ...
    if not cfg.get("cached_text", False):
        ...
        model_t5 = (
            build_module(cfg.t5, MODELS, device_map=device, torch_dtype=dtype)
            .eval()
            .requires_grad_(False)
        )
        log_cuda_memory("t5")
        ...
        model_clip = (
            build_module(cfg.clip, MODELS, device_map=device, torch_dtype=dtype)
            .eval()
            .requires_grad_(False)
        )
    ...
    if cfg.get("cached_text", False):
        ...
        null_vec = torch.load(
            "/mnt/ddn/sora/tmp_load/null_clip.pt", map_location=device
        )
    else:
        null_txt = model_t5("")
        null_vec = model_clip("")
    ...
    logger.info(
        "Training for %s epochs with %s steps per epoch",
        cfg_epochs,
        num_steps_per_epoch,
    )
    ...
    if cfg.get("load", None) is not None:
        ...
        logger.info(
            "Loaded checkpoint %s at epoch %s step %s", cfg.load, ret[0], ret[1]
        )
        ...
        if cfg.get("update_warmup_steps", False):
            ...
            logger.info(
                "The learning rate starts from %s", optimizer.param_groups[0]["lr"]
            )
    ...
    if cfg.get("warmup_ae", False):
        shapes = bucket_to_shapes(
            cfg.get("bucket_config", None), batch_size=cfg.ae.batch_size
        )
    ...
    def prepare_inputs(batch):
        ...
        if cfg.get("dropout_ratio", None) is not None:
            ...
            if num_pad_null_txt > 0:
                cur_null_txt = torch.cat(
                    [cur_null_txt] + [cur_null_txt[:, -1:]] * num_pad_null_txt, dim=1
                )
        ...
        x_t = (
            t_rev[:, None, None] * x_0
            + (1 - (1 - sigma_min) * t_rev[:, None, None]) * x_1
        )
        inp["img"] = x_t
        inp["timesteps"] = t.to(dtype)
        inp["guidance"] = torch.full(
            (x_t.shape[0],), cfg.get("guidance", 4), device=x_t.device, dtype=x_t.dtype
        )
    ...
    def run_iter(inp, x_0, x_1):
        if is_pipeline_enabled(plugin_type, plugin_config):
            inp["target"] = (
                1 - sigma_min
            ) * x_1 - x_0  # follow MovieGen, modify V_t accordingly
            with nsys.range("forward-backward"), timers["forward-backward"]:
                data_iter = iter([inp])
                if cfg.get("no_i2v_ref_loss", False):
                    loss_fn = (
                        lambda out, input_: get_batch_loss(
                            out, input_["target"], input_.pop("masks", None)
                        )
                        / accumulation_steps
                    )
                else:
                    loss_fn = (
                        lambda out, input_: F.mse_loss(
                            out.float(), input_["target"].float(), reduction="mean"
                        )
                        / accumulation_steps
                    )
                loss = booster.execute_pipeline(data_iter, model, loss_fn, optimizer)[
                    "loss"
                ]
            ...
            with nsys.range("backward"), timers["backward"]:
                ...
                with ctx:
                    booster.backward(
                        loss=(loss / accumulation_steps), optimizer=optimizer
                    )
        ...
        with tqdm(
            ...
            for _ in range(start_step, num_steps_per_epoch):
                ...
                with nsys.range("clean_cache"), timers["clean_cache"]:
                    if (
                        ckpt_every > 0
                        and actual_update_step % ckpt_every == 0
                ...
                with nsys.range("checkpoint"), timers["checkpoint"]:
                    if ckpt_every > 0 and actual_update_step % ckpt_every == 0:
                        ...
                        rm_checkpoints(
                            exp_dir, keep_n_latest=cfg.get("keep_n_latest", -1)
                        )
                        logger.info(
                            "Removed old checkpoints and kept %s latest ones.",
                            cfg.get("keep_n_latest", -1),
                        )
