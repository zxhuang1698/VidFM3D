...
def main():
    ...
    # 'batch_size': 1,
    # 'clip': {'from_pretrained': './ckpts/openai/clip-vit-large-patch14',
    #         'max_length': 77,
    #         'type': 'text_embedder'},
    # 'cond_type': 'i2v_head',
    ...
    #                 'pp_size': 1,                                                                                                                                                                                                                          'sequence_parallelism_mode': 'ring_attn',                                                                                                                                                                                              'sp_size': 8,
    #                 'static_graph': True,                                                                                                                                                                                                                  'tp_size': 1,                                                                                                                                                                                                                          'zero_stage': 2},
    # 'plugin_config_ae': {'overlap_allgather': False,
    #                     'pp_size': 1,
    #                     'sp_size': 1,
    #                     'tp_size': 8,
    #                     'zero_stage': 2},
    # 'prompt': 'A plump pig wallows in a muddy pond on a rustic farm, its pink '
    #         'snout poking out as it snorts contentedly. The camera captures the '
    #         "pig's playful splashes, sending ripples through the water under "
    #         'the midday sun. Wooden fences and a red barn stand in the '
    #         "background, framed by rolling green hills. The pig's muddy coat "
    #         'glistens in the sunlight, showcasing the simple pleasures of its '
    #         'carefree life.',
    ...
    # 'save_dir': 'samples',
    # 'seed': 42,
    # 't5': {'from_pretrained': './ckpts/google/t5-v1_1-xxl',
    #         'max_length': 512,
    #         'shardformer': True,
    #         'type': 'text_embedder'}}
    ...
    if use_t2i2v:
        os.makedirs(
            os.path.join(save_dir, sub_dir, "generated_condition"), exist_ok=True
        )
            ...
            for _, batch in pbar:
                ...
                if use_t2i2v:
                    ...
                    x_cond = api_fn_img(
                        sampling_option_t2i,
                        "t2v",
                        seed=sampling_option.seed + epoch
                        if sampling_option.seed
                        else None,
