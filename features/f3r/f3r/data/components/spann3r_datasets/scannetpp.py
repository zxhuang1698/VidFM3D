...
class Scannetpp(BaseManyViewDataset):
    ...
    def _get_views(self, idx, resolution, rng, attempts=0):
        ...
        cams_metadata_path = osp.join(
            self.ROOT, "data", scene_id, "dslr/nerfstudio/transforms_undistorted.json"
        )
        ...
        train_info_path = osp.join(
            self.ROOT, "data", scene_id, "dslr/train_test_lists.json"
        )
        ...
        filepath_index_mapping = {
            frame["file_path"]: index for index, frame in enumerate(frame_meta_data)
        }
        ...
        while len(imgs_idxs) > 0:
            ...
            # Load image data
            impath = osp.join(
                self.ROOT, "data", scene_id, "dslr/undistorted_images", im_idx
            )
            depthpath = osp.join(
                self.ROOT,
                "data",
