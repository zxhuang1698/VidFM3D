...
class Co3d(BaseManyViewDataset):
    ...
    def load_scene(self, scene_class=None, scene_id=None):
        ...
        with open(osp.join(self.ROOT, f"selected_seqs_{self.split}.json"), "r") as f:
            ...
            if scene_class is not None:
                ...
                scenes = {
                    k: v for k, v in scenes.items() if len(v) > 0
                }  # k is class (apple), v is corresponding list
            ...
            if scene_id is not None:
                ...
                }  # k is class (apple), k2 is instance (110_13051_23361), v2 is list of image idx
    ...
    def _get_views(self, idx, resolution, rng, attempts=0):
        ...
        if self.use_comb and not self.full_video:
            ...
            imgs_idxs = [
                max(0, min(im_idx + rng.integers(-4, 5), last)) for im_idx in frame_idx
            ]
        ...
        while len(imgs_idxs) > 0:
            ...
            impath = osp.join(
                self.ROOT, obj, instance, "images", f"frame{view_idx:06d}.jpg"
            )
            ...
            if mask_bg:
                # load object mask
                maskpath = osp.join(
                    self.ROOT, obj, instance, "masks", f"frame{view_idx:06d}.png"
                )
        ...
        if (
            max_depth_max / max_depth_min > 100.0
            or max_depth_max / max_depth_first > 10.0
        ):
