...
class habitat(BaseManyViewDataset):
    ...
    def load_all_scenes(self, base_dir, num_seq=200):
        ...
        self.scenes = {
            (k, v2): list(range(num_seq)) for k, v in self.scenes.items() for v2 in v
        }
    ...
    def _get_views(self, idx, resolution, rng, attempts=0):
        ...
        while len(imgs_idxs) > 0:
            ...
            depthpath = osp.join(
                self.ROOT, data, scene, f"{seq_id:08}_{im_idx}_depth.exr"
            )
            cam_params_path = osp.join(
                self.ROOT, data, scene, f"{seq_id:08}_{im_idx}_camera_params.json"
            )
