...
class BaseManyViewDataset(BaseStereoViewDataset):
    ...
    def sample_frames(self, img_idxs, rng):
        ...
        thresh = int(
            self.min_thresh + self.train_ratio * (self.max_thresh - self.min_thresh)
        )
        ...
        initial_valid_range = max(
            len(img_indices) // num_frames, len(img_indices) - thresh * (num_frames - 1)
        )
        ...
        while len(selected_indices) < num_frames:
            ...
            next_max_index = min(
                current_index + thresh,
                len(img_indices) - (num_frames - len(selected_indices)),
            )
            possible_indices = [
                i
                for i in range(next_min_index, next_max_index + 1)
