...
class ArkitScene(BaseManyViewDataset):
    ...
    def _get_views(self, idx, resolution, rng, attempts=0):
        ...
        if (
            not osp.exists(image_path)
            or not osp.exists(depth_path)
            or not osp.exists(intrinsics_path)
            or not osp.exists(pose_path)
        ):
        ...
        while len(imgs_idxs) > 0:
            ...
            intrinsics_ = self.get_intrinsic(intrinsics_path, im_idx, scene_id).astype(
                np.float32
            )

            if (
                not osp.exists(impath)
                or not osp.exists(depthpath)
